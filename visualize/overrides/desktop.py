...
class CustomWorkspace(Workspace):
	...
	def build_workspace(self):
		...
		self.sidebar_menu = {"items": self.get_sidebar_menu()}
	...
	def get_sidebar_menu(self):
		item = []
		sidebar_menu = self.doc.sidebar_menu

		for menu in sidebar_menu:
			new_item = menu.as_dict().copy()
			new_item["label"] = _(menu.label) if menu.label else _(menu.name)
...
def get_desktop_page(page):
	...
	"""
	try:
		workspace = CustomWorkspace(loads(page))
		...
		return {
			...
			"sidebar_menu": workspace.sidebar_menu,
		}
	except DoesNotExistError:
...
@frappe.whitelist()
def get_sidebar_items(workspace_name):
	"""Get list of sidebar items for desk"""
	has_access = "Workspace Manager" in frappe.get_roles()

	# logging to get value workspace_name from the request
	frappe.log_error("Workspace Name: {}".format(workspace_name))
	...
	# filter by workspace_name from parameter
	filters = {
	...
	order_by = "idx asc"
	fields = ["idx", "label", "parent_menu", "type", "link_type", "link_to", "public", "icon", "workspace_name"]
	all_pages = frappe.get_all(
		"Sidebar Menu", fields=fields, filters=filters, order_by=order_by, ignore_permissions=True
	)
	pages = []
	private_pages = []
